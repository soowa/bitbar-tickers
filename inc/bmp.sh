...
# Candle stick
# candle high low opening closing
candle() {
    mw=11 # max width
    mh=50 # max height
    spc=1 # spacing
    ...
    if [[ $(bc <<< "$op < $as") -eq 1 ]] ; then
        local y=$(bc <<< "(($mh-4) *  (($op-$lo)*100/($hi-$lo))/100) + $spc")
        local h=$(bc <<< "(($mh-4) *  (($as-$lo)*100/($hi-$lo))/100) - $y + $spc")
        fillcol=(00 ff 00 ff)
    else
        local y=$(bc <<< "(($mh-4) *  (($as-$lo)*100/($hi-$lo))/100) + $spc")
        local h=$(bc <<< "(($mh-4) *  (($op-$lo)*100/($hi-$lo))/100) - $y + $spc")
        fillcol=(00 00 ff ff)
    fi
    ...
    curcol=(${fillcol[@]})
    fill $spc $y $(($mw - $spc*2)) $h

    curcol=(${fgcol[@]})
    rect $spc $y $(($mw - $spc*2)) $h
}
OLDIFS=$IFS
